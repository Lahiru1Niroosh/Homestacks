...
const ReportSection2 = () => {
  const [leastDecreasedItem, setLeastDecreasedItem] = useState(null);
  ...
  useEffect(() => {
    const fetchDecreasedItems = async () => {
      try {
        ...
        const leastRes = await axios.get(
          `http://localhost:5000/api/items/min-decrease/${currentUser.id}/${today}`
        );
        setLeastDecreasedItem(leastRes.data);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div>
      {loading ? (
        ...
        leastDecreasedItem &&
        renderItemCard(
          "Least Consumed Item ( " + formattedDate + " )",
          leastDecreasedItem
        )
      )}
    </div>
